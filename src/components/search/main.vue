<template>
  <div class="d-flex flex-align-center department-search-container">
    ...
    <p class="search-input-box">
      <input  :style="{'width':width + 'px'}" type="text" v-model="searchVal" :placeholder="placeholder" @click="isKeyboard = !isKeyboard" readonly="readonly" />
      <img class="clean-btn" src="../../assets/search/clean@2x.png" v-if="isShow" @click="operation(1)"/>
    </p>
    <p class="search-tips" @click="operation(2)">{{searchText ? searchText : '搜索'}}</p>

    <!-- 键盘 -->
    <all-lock @keyboardNum="keyboardNum" :otherList="otherList" :imageShow="imageShow" v-show="isKeyboard"></all-Lock>
  </div>
</template>
